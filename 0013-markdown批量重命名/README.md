...
```bash
python main.py ./vuepress-project/docs ./output
```
...
├── markdown/
│   ├── README__xxxxxx.md
│   ├── guide__install__xxxxxx.md
│   ├── guide__config__xxxxxx.md
│   └── api__user__xxxxxx.md
│
├── images/
│   └── install__xxxxxxxx.png
│
└── manifest.json
```
...
![安装流程](images/install__a1b2c3d4.png)
```

### 我建议下一步继续升级

这个版本是一个**基础版 V1**。真正接入 AI 知识库时，我建议继续增加：

1. **自动删除 VuePress Frontmatter**
2. **忽略 `.vuepress`、`node_modules` 等目录**
3. **支持图片 OCR**
4. **支持调用视觉模型生成图片描述**
5. **按 `# / ## / ###` 自动切分 Markdown**
6. **控制每个 Chunk 的长度，例如 500～1000 tokens**
7. **生成带 metadata 的 JSONL**
8. **直接导入 Milvus / Qdrant**
9. **支持增量更新，只处理修改过的 Markdown**
...
如果你准备继续做完整的 AI 知识库，我建议下一步我直接给你升级成 **一个完整 Python 项目**，采用模块化结构，例如 `scanner.py`、`image_processor.py`、`markdown_processor.py`、`manifest.py`，而不是所有代码都放在一个 `main.py` 里。
